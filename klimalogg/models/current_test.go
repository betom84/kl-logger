...
func TestUnmarshalCurrentWeatherData(t *testing.T) {
	...
	assert.Equal(t, 55, model.Humidity[0])
	assert.Equal(t, 32, model.HumidityMin[0])
	assert.Equal(t, 98, model.HumidityMax[0])
	...
	assert.Equal(t, 42, model.Humidity[0])
	assert.Equal(t, 40, model.HumidityMin[0])
	assert.Equal(t, 62, model.HumidityMax[0])
